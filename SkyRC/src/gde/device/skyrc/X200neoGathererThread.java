 ...
 */
public class X200neoGathererThread extends Thread {
	protected static final int	USB_QUERY_DELAY	= GDE.IS_WINDOWS ? 70 : 160;
	final static String	$CLASS_NAME									= X200neoGathererThread.class.getName();
	final static Logger	log													= Logger.getLogger(X200neoGathererThread.class.getName());
	...
	public void run() {
		try {
			...
			if (X200neoGathererThread.log.isLoggable(Level.FINE))
				X200neoGathererThread.log.logp(Level.FINE, X200neoGathererThread.$CLASS_NAME, $METHOD_NAME, "====> entry initial time step ms = " + this.device.getTimeStep_ms()); //$NON-NLS-1$
...
//							this.retryCounter	= Q200GathererThread.WAIT_TIME_RETRYS;	//60 Min
						...
						log.logp(Level.FINE, X200neoGathererThread.$CLASS_NAME, $METHOD_NAME, "wait for device activation ..."); //$NON-NLS-1$
					...
					if (e instanceof TimeOutException) {
						this.application.setStatusMessage(Messages.getString(MessageIds.GDE_MSGI3600));
						if (X200neoGathererThread.log.isLoggable(Level.FINE))
							X200neoGathererThread.log.logp(Level.FINE, X200neoGathererThread.$CLASS_NAME, $METHOD_NAME, Messages.getString(MessageIds.GDE_MSGI3600));
					}
					...
					// program end or unexpected exception occurred, stop data gathering to enable save data by user
						...
						X200neoGathererThread.log.log(Level.FINE, "data gathering end detected"); //$NON-NLS-1$
			...
			if (X200neoGathererThread.log.isLoggable(Level.FINE)) X200neoGathererThread.log.logp(Level.FINE, X200neoGathererThread.$CLASS_NAME, $METHOD_NAME, "======> exit"); //$NON-NLS-1$
			...
			try {
				if (this.usbInterface != null) {
					...
					X200neoGathererThread.log.log(Level.FINE, "USB interface closed");
				}
			}
			catch (UsbException e) {
				X200neoGathererThread.log.log(Level.SEVERE, e.getMessage(), e);
			}
		}
	}
	...
	private Object[] processDataChannel(final int number, RecordSet recordSet, String processRecordSetKey, final byte[] channelBuffer, final byte[] dataBuffer, final int[] points) throws DataInconsitsentException {
		...
		if (X200neoGathererThread.log.isLoggable(Level.FINE))
			X200neoGathererThread.log.log(Level.FINE, "Channel = " + number + " : processTypeName = " + processTypeName + " processSubType = " + processSubTypeName);
		...
		if (channel != null) {
			...
			if (recordSet == null || !processRecordSetKey.contains(processTypeName) || !processRecordSetKey.contains(processSubTypeName)) {
				...
				if (X200neoGathererThread.log.isLoggable(Level.FINE)) {
					X200neoGathererThread.log.log(Level.FINE, number + " : processRecordSetKey = " + processRecordSetKey);
				}
				...
				X200neoGathererThread.log.logp(Level.FINE, X200neoGathererThread.$CLASS_NAME, $METHOD_NAME, processRecordSetKey + " created for channel " + channel.getName()); //$NON-NLS-1$
...
//			}
	...
	void stopDataGatheringThread(boolean enableEndMessage, Throwable throwable) {
		...
		if (throwable != null) {
			X200neoGathererThread.log.logp(Level.WARNING, X200neoGathererThread.$CLASS_NAME, $METHOD_NAME, throwable.getMessage(), throwable);
		}

		this.isCollectDataStopped = true;

		if (this.usbPort != null && this.usbPort.getXferErrors() > 0) {
			X200neoGathererThread.log.log(Level.WARNING, "During complete data transfer " + this.usbPort.getXferErrors() + " number of errors occured!"); //$NON-NLS-1$ //$NON-NLS-2$
		}
		if (this.usbPort != null && this.usbPort.isConnected() && this.isPortOpenedByLiveGatherer == true && this.usbPort.isConnected()) {
			try {
				...
				X200neoGathererThread.log.log(Level.WARNING, e.getMessage(), e);
			}
		}
...
//		}
	...
	void finalizeRecordSet(boolean doClosePort) {
		...
		if (tmpRecordSet != null) {
			...
			X200neoGathererThread.log.log(Level.TIME, "set average time step msec = " + this.device.getAverageTimeStep_ms()); //$NON-NLS-1$
		}
	}
