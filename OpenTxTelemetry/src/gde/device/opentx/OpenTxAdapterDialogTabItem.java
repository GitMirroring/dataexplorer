 ...
 */
public class OpenTxAdapterDialogTabItem extends CTabItem {
	final static Logger			log								= Logger.getLogger(OpenTxAdapterDialogTabItem.class.getName());
	...
	public OpenTxAdapterDialogTabItem(CTabFolder parentTabFolder, OpenTxAdapterDialog parentDialog, int useChannelConfigNumber, IDevice useDevice) {
...

		{
			...
			this.scolledComposite.addControlListener(new ControlListener() {
				...
				public void controlResized(ControlEvent evt) {
					OpenTxAdapterDialogTabItem.log.log(java.util.logging.Level.FINEST, "scolledComposite.controlResized, event=" + evt); //$NON-NLS-1$
					int height = 35 + OpenTxAdapterDialogTabItem.this.device.getChannelMeasuremtsReplacedNames(OpenTxAdapterDialogTabItem.this.parent.getSelectionIndex() + 1).size() * 28 / 2;
					Channel channel = Channels.getInstance().get(OpenTxAdapterDialogTabItem.this.parent.getSelectionIndex() + 1);
					if (channel != null)
						if (channel.getActiveRecordSet() != null)
							height = 35 + (channel.getActiveRecordSet().size() + 1) * 28 / 2;
					OpenTxAdapterDialogTabItem.this.mainTabComposite.setSize(OpenTxAdapterDialogTabItem.this.scolledComposite.getClientArea().width, height);
				}

				@Override
				public void controlMoved(ControlEvent evt) {
					OpenTxAdapterDialogTabItem.log.log(java.util.logging.Level.FINEST, "scolledComposite.controlMoved, event=" + evt); //$NON-NLS-1$
					int height = 35 + OpenTxAdapterDialogTabItem.this.device.getChannelMeasuremtsReplacedNames(OpenTxAdapterDialogTabItem.this.parent.getSelectionIndex() + 1).size() * 28 / 2;
					Channel channel = Channels.getInstance().get(OpenTxAdapterDialogTabItem.this.parent.getSelectionIndex() + 1);
					if (channel != null)
						if (channel.getActiveRecordSet() != null)
