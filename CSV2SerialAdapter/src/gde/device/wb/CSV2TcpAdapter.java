 ...
 */
public class CSV2TcpAdapter extends DeviceConfiguration implements IDevice {
	final static Logger							log					= Logger.getLogger(CSV2TcpAdapter.class.getName());
	 ...
	 */
	public CSV2TcpAdapter(String deviceProperties) throws FileNotFoundException, JAXBException {
	 ...
	 */
	public CSV2TcpAdapter(DeviceConfiguration deviceConfig) {
		super(deviceConfig);
		// initializing the resource bundle for this device
		Messages.setDeviceResourceBundle("gde.device.wb.messages", Settings.getInstance().getLocale(), this.getClass().getClassLoader()); //$NON-NLS-1$
	 ...
	 */
	@Override
	public CSV2TcpAdapterDialog getDialog() {
		return this.dialog;
	}
	...
	public void importCsvFiles() {
		...
		Thread reader = new Thread("reader") { //$NON-NLS-1$
			...
			public void run() {
				try {
					CSV2TcpAdapter.this.application.setPortConnected(true);
					...
					CSV2TcpAdapter.this.application.setPortConnected(false);
				}
			}
		};
		reader.start();
