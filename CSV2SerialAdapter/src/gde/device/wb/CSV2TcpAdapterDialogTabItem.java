 ...
 */
public class CSV2TcpAdapterDialogTabItem extends CTabItem {
	final static Logger							log									= Logger.getLogger(CSV2TcpAdapterDialogTabItem.class.getName());
	...
	public CSV2TcpAdapterDialogTabItem(CTabFolder parentTabFolder, CSV2TcpAdapterDialog parentDialog, int useChannelConfigNumber, CSV2TcpAdapter useDevice) {
			...
			this.buttonComposite.setLayout(buttonCompositeLayout);
			{
				...
				this.inputFileButton.addSelectionListener(new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent evt) {
						...
						if (CSV2TcpAdapterDialogTabItem.this.isVisibilityChanged) {
							String msg = Messages.getString(gde.messages.MessageIds.GDE_MSGI0041, new String[] {CSV2TcpAdapterDialogTabItem.this.device.getPropertiesFileName()});
							if (CSV2TcpAdapterDialogTabItem.this.application.openYesNoMessageDialog(CSV2TcpAdapterDialogTabItem.this.dialog.getDialogShell(), msg) == SWT.YES) {
								log.log(java.util.logging.Level.FINE, "SWT.YES"); //$NON-NLS-1$
								CSV2TcpAdapterDialogTabItem.this.device.storeDeviceProperties();
							}
						}
						CSV2TcpAdapterDialogTabItem.this.device.importCsvFiles();
					}
				});
			}
		}
