 ...
 */
public class FutabaAdapter extends DeviceConfiguration implements IDevice {
	final static Logger				log					= Logger.getLogger(FutabaAdapter.class.getName());
	 ...
	 */
	public FutabaAdapter(String xmlFileName) throws FileNotFoundException, JAXBException {
		super(xmlFileName);
		// initializing the resource bundle for this device
		Messages.setDeviceResourceBundle("gde.device.robbe.messages", Settings.getInstance().getLocale(), this.getClass().getClassLoader()); //$NON-NLS-1$

		this.application = DataExplorer.getInstance();
		this.channels = Channels.getInstance();
		this.dialog = new FutabaAdapterDialog(this.application.getShell(), this);
	 ...
	 * @throws JAXBException
	 */
	public FutabaAdapter(String xmlFileName, Unmarshaller tmpUnmarshaller) throws FileNotFoundException, JAXBException {
	 ...
	 * @param deviceConfig
	 */
	public FutabaAdapter(DeviceConfiguration deviceConfig) {
		super(deviceConfig);
		// initializing the resource bundle for this device
		Messages.setDeviceResourceBundle("gde.device.robbe.messages", Settings.getInstance().getLocale(), this.getClass().getClassLoader()); //$NON-NLS-1$
	...
	private void readProperties() {
		...
		try (InputStream stream = DataAccess.getInstance().getMappingInputStream("FutabaMeasurementMappings.xml")) {
			...
			String preopertyFilePath = Settings.MAPPINGS_DIR_NAME + GDE.STRING_FILE_SEPARATOR_UNIX + "FutabaMeasurementMappings.xml"; //$NON-NLS-1$
	 ...
	 */
	@Override
	public FutabaAdapterDialog getDialog() {
		return this.dialog;
	}
	...
	public void addDataBufferAsRawDataPoints(RecordSet recordSet, byte[] dataBuffer, int recordDataSize, boolean doUpdateProgressBar) throws DataInconsitsentException {
		...
		FutabaAdapter.log.log(java.util.logging.Level.FINE, timeStamps.size() + " timeStamps = " + timeStamps.toString()); //$NON-NLS-1$

		for (int i = 0; i < recordDataSize; i++) {
			FutabaAdapter.log.log(java.util.logging.Level.FINER, i + " i*dataBufferSize+timeStampBufferSize = " + i * dataBufferSize + timeStampBufferSize); //$NON-NLS-1$
	...
	public double translateValue(Record record, double value) {
		...
		FutabaAdapter.log.log(java.util.logging.Level.FINE, "for " + record.getName() + " in value = " + value + " out value = " + newValue); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		return newValue;
	}
	...
	public void updateVisibilityStatus(RecordSet recordSet, boolean includeReasonableDataCheck) {
		...
		for (int i = 0; i < recordSet.size(); ++i) {
			...
			if (FutabaAdapter.log.isLoggable(java.util.logging.Level.FINE)) FutabaAdapter.log.log(java.util.logging.Level.FINE, record.getName() + " = " + measurementNames[i]); //$NON-NLS-1$
			...
			if (record.isActive() && record.isDisplayable()) {
				if (FutabaAdapter.log.isLoggable(java.util.logging.Level.FINE)) FutabaAdapter.log.log(java.util.logging.Level.FINE, "add to displayable counter: " + record.getName()); //$NON-NLS-1$
				++displayableCounter;
			}
		}
	...
	public void importCsvFiles() {
		...
		Thread reader = new Thread("reader") { //$NON-NLS-1$
			...
			public void run() {
				try {
					FutabaAdapter.this.application.setPortConnected(true);
					for (String tmpFileName : fd.getFileNames()) {
						String selectedImportFile = fd.getFilterPath() + GDE.STRING_FILE_SEPARATOR_UNIX + tmpFileName;
						FutabaAdapter.log.log(java.util.logging.Level.FINE, "selectedImportFile = " + selectedImportFile); //$NON-NLS-1$
						...
						if (fd.getFileName().length() > 4) {
							try {
								...
								FutabaAdapter.log.log(java.util.logging.Level.WARNING, e.getMessage(), e);
							}
						}
					}
				}
				finally {
					FutabaAdapter.this.application.setPortConnected(false);
				}
			}
		};
		reader.start();
