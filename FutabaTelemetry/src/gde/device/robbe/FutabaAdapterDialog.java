 ...
 */
public class FutabaAdapterDialog extends DeviceDialog {
	final static Logger	log									= Logger.getLogger(FutabaAdapterDialog.class.getName());
	...
	final FutabaAdapter		device;																																	// get device specific things, get serial port, ...
	 ...
	 */
	public FutabaAdapterDialog(Shell parent, FutabaAdapter useDevice) {
		super(parent);
	...
	public void open() {
		try {
			...
			FutabaAdapterDialog.log.log(Level.FINE, "dialogShell.isDisposed() " + ((this.dialogShell == null) ? "null" : this.dialogShell.isDisposed())); //$NON-NLS-1$ //$NON-NLS-2$
			if (this.dialogShell == null || this.dialogShell.isDisposed()) {
				...
				this.dialogShell.addListener(SWT.Traverse, new Listener() {
					...
					public void handleEvent(Event event) {
						...
						case SWT.TRAVERSE_ESCAPE:
							FutabaAdapterDialog.this.dialogShell.close();
				...
				this.dialogShell.addDisposeListener(new DisposeListener() {
					...
					public void widgetDisposed(DisposeEvent evt) {
						FutabaAdapterDialog.log.log(Level.FINEST, "dialogShell.widgetDisposed, event=" + evt); //$NON-NLS-1$
						if (FutabaAdapterDialog.this.device.isChangePropery()) {
							String msg = Messages.getString(gde.messages.MessageIds.GDE_MSGI0041, new String[] { FutabaAdapterDialog.this.device.getPropertiesFileName() });
							if (FutabaAdapterDialog.this.application.openYesNoMessageDialog(getDialogShell(), msg) == SWT.YES) {
								FutabaAdapterDialog.log.log(Level.FINE, "SWT.YES"); //$NON-NLS-1$
								FutabaAdapterDialog.this.device.storeDeviceProperties();
								setClosePossible(true);
							}
						}
						FutabaAdapterDialog.this.dispose();
					}
				});
				this.dialogShell.addHelpListener(new HelpListener() {
					@Override
					public void helpRequested(HelpEvent evt) {
						FutabaAdapterDialog.log.log(Level.FINER, "dialogShell.helpRequested, event=" + evt); //$NON-NLS-1$
						FutabaAdapterDialog.this.application.openHelpDialog(FutabaAdapterDialog.this.device.getName(), "HelpInfo.html"); //$NON-NLS-1$
					}
				});
				this.dialogShell.addPaintListener(new PaintListener() {
					@Override
					public void paintControl(PaintEvent paintevent) {
						if (FutabaAdapterDialog.log.isLoggable(Level.FINEST)) FutabaAdapterDialog.log.log(Level.FINEST, "dialogShell.paintControl, event=" + paintevent); //$NON-NLS-1$
						RecordSet activeRecordSet = FutabaAdapterDialog.this.application.getActiveRecordSet();
						if (FutabaAdapterDialog.this.lastActiveRecordSet == null && activeRecordSet != null
								|| (activeRecordSet != null && !FutabaAdapterDialog.this.lastActiveRecordSet.getName().equals(activeRecordSet.getName()))) {
							FutabaAdapterDialog.this.tabFolder.setSelection(Channels.getInstance().getActiveChannelNumber() - 1);
						}
						FutabaAdapterDialog.this.lastActiveRecordSet = FutabaAdapterDialog.this.application.getActiveRecordSet();
					}
				});
				{
					...
					this.tabFolder.setSimple(false);
					{
						for (int i = 1; i <= this.device.getChannelCount(); i++) {
							new FutabaAdapterDialogTabItem(this.tabFolder, this, i, this.device);
						}
					}
				...
				}
				{
					...
					this.saveVisualizationButton.addSelectionListener(new SelectionAdapter() {
						...
						public void widgetSelected(SelectionEvent evt) {
							FutabaAdapterDialog.log.log(Level.FINEST, "saveButton.widgetSelected, event=" + evt); //$NON-NLS-1$
							FutabaAdapterDialog.this.device.storeDeviceProperties();
							FutabaAdapterDialog.this.saveVisualizationButton.setEnabled(false);
						}
					});
				}
				{
					...
					this.inputFileButton.addSelectionListener(new SelectionAdapter() {
						@Override
						public void widgetSelected(SelectionEvent evt) {
							FutabaAdapterDialog.log.log(Level.FINEST, "inputFileButton.widgetSelected, event=" + evt); //$NON-NLS-1$
							if (FutabaAdapterDialog.this.isVisibilityChanged) {
								String msg = Messages.getString(gde.messages.MessageIds.GDE_MSGI0041, new String[] { FutabaAdapterDialog.this.device.getPropertiesFileName() });
								if (FutabaAdapterDialog.this.application.openYesNoMessageDialog(FutabaAdapterDialog.this.dialogShell, msg) == SWT.YES) {
									FutabaAdapterDialog.log.log(Level.FINE, "SWT.YES"); //$NON-NLS-1$
									FutabaAdapterDialog.this.device.storeDeviceProperties();
								}
							}
							FutabaAdapterDialog.this.device.open_closeCommPort();
						}
					});
				}
				{
					...
					this.helpButton.addSelectionListener(new SelectionAdapter() {
						@Override
						public void widgetSelected(SelectionEvent evt) {
							FutabaAdapterDialog.log.log(Level.FINEST, "helpButton.widgetSelected, event=" + evt); //$NON-NLS-1$
							FutabaAdapterDialog.this.application.openHelpDialog(FutabaAdapterDialog.this.device.getName(), "HelpInfo.html"); //$NON-NLS-1$
						}
					});
				}
				{
					...
					this.closeButton.addSelectionListener(new SelectionAdapter() {
						@Override
						public void widgetSelected(SelectionEvent evt) {
							FutabaAdapterDialog.log.log(Level.FINEST, "closeButton.widgetSelected, event=" + evt); //$NON-NLS-1$
							FutabaAdapterDialog.this.dispose();
						}
					});
				}
			...
			FutabaAdapterDialog.log.log(Level.SEVERE, e.getMessage(), e);
		}
	}
