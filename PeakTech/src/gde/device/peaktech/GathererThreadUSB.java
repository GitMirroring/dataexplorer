/**************************************************************************************
    ...
    Copyright (c) 2024,2025,2026 Winfried Bruegmann
****************************************************************************************/
 ...
 */
public class GathererThreadUSB extends Thread {

	final static String				$CLASS_NAME									= GathererThreadUSB.class.getName();
	final static Logger				log													= Logger.getLogger(GathererThreadUSB.class.getName());

	DataExplorer							application;
	final PeakTechUsbPort			usbPort;
	final PeakTechUSB					device;
	final PeakTechDialog			dialog;
	...
	int												retryCounter								= GathererThreadUSB.WAIT_TIME_RETRYS;													// 600 * 1 sec timeout = 600 sec
	 ...
	 * @throws Exception
	 */
	public GathererThreadUSB(DataExplorer currentApplication, PeakTech useDevice, PeakTechUsbPort useSerialPort, int channelConfigNumber, PeakTechDialog useDialog)
			throws ApplicationConfigurationException, SerialPortException {
		super("liveDataGatherer");
		this.application = currentApplication;
		this.device = (PeakTechUSB) useDevice;
	...
	public void run() {
		...
		if (log.isLoggable(Level.FINE))
			log.logp(Level.FINE, GathererThreadUSB.$CLASS_NAME, $METHOD_NAME, "====> entry " + "initial time step ms = " + this.device.getTimeStep_ms()); //$NON-NLS-1$ //$NON-NLS-2$
		this.application.setStatusMessage(""); //$NON-NLS-1$

		try {
			this.usbPort.read(new byte[8], 1000);
			this.usbPort.read(new byte[8], 1000);
		...
		while (!this.isCollectDataStopped) {
			try {
				...
				m_unit = configData.get(PeakTech.INPUT_UNIT);
				...
				if (m_unit.length() > 0) {
					...
					if (this.channel.size() == 0 || recordSet == null || !this.recordSetKey.endsWith(" " + processName) || !m_unit.equals(old_unit)) { //$NON-NLS-1$
						this.application.setStatusMessage(""); //$NON-NLS-1$
						setRetryCounter(GathererThreadUSB.WAIT_TIME_RETRYS); // 600 * receive timeout sec timeout  600 sec
						...
						if (log.isLoggable(Level.FINE))
							log.logp(Level.FINE, GathererThreadUSB.$CLASS_NAME, $METHOD_NAME, this.recordSetKey + " created for channel " + this.channel.getName()); //$NON-NLS-1$
						...
						record.setSymbol(configData.get(PeakTech.INPUT_SYMBOL));
						record.setName(configData.get(PeakTech.INPUT_TYPE));
					...
					if (this.cycleTime_ms > 1500 || lastTimeStamp == 0) {
						if ((tmpCycleTime - lastTimeStamp) > (this.cycleTime_ms - 150)) {
							...
							if (log.isLoggable(Level.TIME))
								log.logp(Level.TIME, GathererThreadUSB.$CLASS_NAME, $METHOD_NAME,
					...
					if (recordSet.isChildOfActiveChannel() && recordSet.equals(this.channels.getActiveChannel().getActiveRecordSet())) {
						GathererThreadUSB.this.application.updateAllTabs(false);
					}
				}
			}
			catch (DataInconsitsentException e) {
				GathererThreadUSB.log.log(Level.SEVERE, Messages.getString(gde.messages.MessageIds.GDE_MSGE0036, new Object[] { this.getClass().getSimpleName(), $METHOD_NAME }));
				postCleanup();
			}
			catch (Throwable e) {
				GathererThreadUSB.log.logp(Level.WARNING, GathererThreadUSB.$CLASS_NAME, $METHOD_NAME, e.getMessage(), e);
				if (e instanceof TimeOutException) {
					this.application.setStatusMessage(Messages.getString(MessageIds.GDE_MSGI1500));
					GathererThreadUSB.log.logp(Level.WARNING, GathererThreadUSB.$CLASS_NAME, $METHOD_NAME, "wait for device activation ... " + getRetryCounter()); //$NON-NLS-1$
					if (0 == (setRetryCounter(getRetryCounter() - 1))) {
						GathererThreadUSB.log.log(Level.WARNING, "device activation timeout", e); //$NON-NLS-1$
		...
		configData.clear();
		if (log.isLoggable(Level.FINE))
			log.logp(Level.FINE, GathererThreadUSB.$CLASS_NAME, $METHOD_NAME, "======> exit"); //$NON-NLS-1$
	}
	...
	void stopDataGatheringThread(boolean enableEndMessage) {
		...
		if (this.usbPort != null && this.usbPort.getXferErrors() > 0) {
			GathererThreadUSB.log.log(Level.WARNING, "During complete data transfer " + this.usbPort.getXferErrors() + " number of errors occured!"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	void finalizeRecordSet(boolean doClosePort) {
		if (doClosePort && this.isPortOpenedByLiveGatherer && this.usbPort.isConnected()) this.usbPort.close();
		...
		if (this.channel.get(this.recordSetKey) != null) {
			...
			if (Thread.currentThread().threadId() == this.application.getThreadId()) {
				...
				GDE.display.asyncExec(new Runnable() {
					@Override
					public void run() {
						GathererThreadUSB.this.application.getMenuToolBar().updateRecordSetSelectCombo();
						GathererThreadUSB.this.application.updateStatisticsData();
						GathererThreadUSB.this.application.updateDataTable(useRecordSetKey, true);
						GathererThreadUSB.this.device.getDialog().resetButtons();
					}
				});
			}
		}
