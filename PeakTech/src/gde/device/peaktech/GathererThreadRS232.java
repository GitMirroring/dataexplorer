 ...
 */
public class GathererThreadRS232 extends Thread {

	final static String				$CLASS_NAME									= GathererThreadRS232.class.getName();
	final static Logger				log													= Logger.getLogger(GathererThreadRS232.class.getName());

	DataExplorer							application;
	final PeakTechSerialPort	serialPort;
	final PeakTechRS232				device;
	...
	int												retryCounter								= GathererThreadRS232.WAIT_TIME_RETRYS;													// 600 * 1 sec timeout = 600 sec
	 ...
	 * @throws Exception
	 */
	public GathererThreadRS232(DataExplorer currentApplication, PeakTech useDevice, PeakTechSerialPort useSerialPort, int channelConfigNumber, PeakTechDialog useDialog)
			throws ApplicationConfigurationException, SerialPortException {
		super("liveDataGatherer");
		this.application = currentApplication;
		this.device = (PeakTechRS232) useDevice;
	...
	public void run() {
		...
		if (log.isLoggable(Level.FINE))
			log.logp(Level.FINE, GathererThreadRS232.$CLASS_NAME, $METHOD_NAME, "====> entry " + "initial time step ms = " + this.device.getTimeStep_ms()); //$NON-NLS-1$ //$NON-NLS-2$
			...
			try {
				...
				if (m_unit.length() > 0) {
					...
					if (this.channel.size() == 0 || recordSet == null || !this.recordSetKey.endsWith(" " + processName) || !m_unit.equals(old_unit)) { //$NON-NLS-1$
						this.application.setStatusMessage(""); //$NON-NLS-1$
						setRetryCounter(GathererThreadRS232.WAIT_TIME_RETRYS); // 600 * receive timeout sec timeout  600 sec
						...
						if (log.isLoggable(Level.FINE))
							log.logp(Level.FINE, GathererThreadRS232.$CLASS_NAME, $METHOD_NAME, this.recordSetKey + " created for channel " + this.channel.getName()); //$NON-NLS-1$
					...
					if (this.cycleTime_ms > 1500 || lastTimeStamp == 0) {
						if ((tmpCycleTime - lastTimeStamp) > (this.cycleTime_ms - 150)) {
							...
							if (log.isLoggable(Level.TIME))
								log.logp(Level.TIME, GathererThreadRS232.$CLASS_NAME, $METHOD_NAME,
					...
					if (recordSet.isChildOfActiveChannel() && recordSet.equals(this.channels.getActiveChannel().getActiveRecordSet())) {
						GathererThreadRS232.this.application.updateAllTabs(false);
					}
				}
			}
			catch (DataInconsitsentException e) {
				GathererThreadRS232.log.log(Level.SEVERE, Messages.getString(gde.messages.MessageIds.GDE_MSGE0036, new Object[] { this.getClass().getSimpleName(), $METHOD_NAME }));
				postCleanup();
			}
			catch (Throwable e) {
				GathererThreadRS232.log.logp(Level.WARNING, GathererThreadRS232.$CLASS_NAME, $METHOD_NAME, e.getMessage(), e);
				if (e instanceof TimeOutException) {
					this.application.setStatusMessage(Messages.getString(MessageIds.GDE_MSGI1500));
					GathererThreadRS232.log.logp(Level.WARNING, GathererThreadRS232.$CLASS_NAME, $METHOD_NAME, "wait for device activation ... " + getRetryCounter()); //$NON-NLS-1$
					if (0 == (setRetryCounter(getRetryCounter() - 1))) {
						GathererThreadRS232.log.log(Level.WARNING, "device activation timeout", e); //$NON-NLS-1$
		...
		configData.clear();
		if (log.isLoggable(Level.FINE))
			log.logp(Level.FINE, GathererThreadRS232.$CLASS_NAME, $METHOD_NAME, "======> exit"); //$NON-NLS-1$
	}
	...
	void stopDataGatheringThread(boolean enableEndMessage) {
		...
		if (this.serialPort != null && this.serialPort.getXferErrors() > 0) {
			GathererThreadRS232.log.log(Level.WARNING, "During complete data transfer " + this.serialPort.getXferErrors() + " number of errors occured!"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		if (this.channel.get(this.recordSetKey) != null) {
			...
			if (Thread.currentThread().threadId() == this.application.getThreadId()) {
				...
				GDE.display.asyncExec(new Runnable() {
					@Override
					public void run() {
						GathererThreadRS232.this.application.getMenuToolBar().updateRecordSetSelectCombo();
						GathererThreadRS232.this.application.updateStatisticsData();
						GathererThreadRS232.this.application.updateDataTable(useRecordSetKey, true);
						GathererThreadRS232.this.device.getDialog().resetButtons();
					}
				});
			}
		}
