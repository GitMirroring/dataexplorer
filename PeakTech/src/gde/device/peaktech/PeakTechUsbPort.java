 ...
 */
public class PeakTechUsbPort extends DeviceCommPort implements IDeviceCommPort {
	final static String				$CLASS_NAME	= PeakTechUsbPort.class.getName();
	...
	public synchronized byte[] getData(final UsbInterface iface) throws Exception {
		...
		try {
			...
			if (log.isLoggable(Level.OFF)) {
				log.logp(Level.OFF, $CLASS_NAME, $METHOD_NAME, StringHelper.byte2Hex2CharString(data, data.length));
			}
		}
		catch (Exception e) {
