 ...
 */
public class ZeroAdapterDialogTabItem extends CTabItem {
	final static Logger			log								= Logger.getLogger(ZeroAdapterDialogTabItem.class.getName());
	...
	public ZeroAdapterDialogTabItem(CTabFolder parentTabFolder, ZeroAdapterDialog parentDialog, int useChannelConfigNumber, IDevice useDevice) {
	...
	void create() {
...

		{
			...
			this.scolledComposite.setContent(this.mainTabComposite);
			{
				...
				this.tabItemLabel.setText(Messages.getString(MessageIds.GDE_MSGT3351));
			}
			{
			...
			this.scolledComposite.addControlListener(new ControlListener() {
				...
				public void controlResized(ControlEvent evt) {
					ZeroAdapterDialogTabItem.log.log(java.util.logging.Level.FINEST, "scolledComposite.controlResized, event=" + evt); //$NON-NLS-1$
					int height = 35 + ZeroAdapterDialogTabItem.this.device.getChannelMeasuremtsReplacedNames(ZeroAdapterDialogTabItem.this.parent.getSelectionIndex() + 1).size() * 28 / 2;
					Channel channel = Channels.getInstance().get(ZeroAdapterDialogTabItem.this.parent.getSelectionIndex() + 1);
					if (channel != null)
						if (channel.getActiveRecordSet() != null)
							height = 35 + (channel.getActiveRecordSet().size() + 1) * 28 / 2;
					ZeroAdapterDialogTabItem.this.mainTabComposite.setSize(ZeroAdapterDialogTabItem.this.scolledComposite.getClientArea().width, height);
				}

				@Override
				public void controlMoved(ControlEvent evt) {
					ZeroAdapterDialogTabItem.log.log(java.util.logging.Level.FINEST, "scolledComposite.controlMoved, event=" + evt); //$NON-NLS-1$
					int height = 35 + ZeroAdapterDialogTabItem.this.device.getChannelMeasuremtsReplacedNames(ZeroAdapterDialogTabItem.this.parent.getSelectionIndex() + 1).size() * 28 / 2;
					Channel channel = Channels.getInstance().get(ZeroAdapterDialogTabItem.this.parent.getSelectionIndex() + 1);
					if (channel != null)
						if (channel.getActiveRecordSet() != null)
