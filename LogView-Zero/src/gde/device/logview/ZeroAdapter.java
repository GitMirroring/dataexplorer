/**************************************************************************************
    ...
    Copyright (c) 2019,2020,2021,2022,2023,2024,2025,2026 Winfried Bruegmann
****************************************************************************************/
...
/**
 * LogView Zero data adapter device implementation class
 */
public class ZeroAdapter extends DeviceConfiguration implements IDevice {
	final static Logger				log					= Logger.getLogger(ZeroAdapter.class.getName());
	final static Properties		properties	= new Properties();

	final DataExplorer				application;
	final Channels						channels;
	final ZeroAdapterDialog		dialog;
	 ...
	 */
	public ZeroAdapter(String xmlFileName) throws FileNotFoundException, JAXBException {
		super(xmlFileName);
		// initializing the resource bundle for this device
		Messages.setDeviceResourceBundle("gde.device.logview.messages", Settings.getInstance().getLocale(), this.getClass().getClassLoader()); //$NON-NLS-1$

		this.application = DataExplorer.getInstance();
		this.channels = Channels.getInstance();
		this.dialog = new ZeroAdapterDialog(this.application.getShell(), this);
		if (this.application.getMenuToolBar() != null) {
			this.configureSerialPortMenu(DeviceCommPort.ICON_SET_IMPORT_CLOSE, Messages.getString(MessageIds.GDE_MSGT3353, new String[] {this.getDataBlockPreferredFileExtention()}), Messages.getString(MessageIds.GDE_MSGT3353, new String[] {this.getDataBlockPreferredFileExtention()}));
	 ...
	 * @throws JAXBException
	 */
	public ZeroAdapter(String xmlFileName, Unmarshaller tmpUnmarshaller) throws FileNotFoundException, JAXBException {
	 ...
	 * @param deviceConfig
	 */
	public ZeroAdapter(DeviceConfiguration deviceConfig) {
		super(deviceConfig);
		// initializing the resource bundle for this device
		Messages.setDeviceResourceBundle("gde.device.logview.messages", Settings.getInstance().getLocale(), this.getClass().getClassLoader()); //$NON-NLS-1$
	...
	public void updateFileImportMenu(Menu importMenue) {
		...
		if (importMenue.getItem(importMenue.getItemCount() - 1).getText().equals(Messages.getString(gde.messages.MessageIds.GDE_MSGT0018))) {
			...
			importDeviceLogItem.setText(Messages.getString(MessageIds.GDE_MSGT3354, new String[] {this.getDataBlockPreferredFileExtention(), GDE.MOD1[0]}));
			importDeviceLogItem.setAccelerator(SWT.MOD1 + Messages.getAcceleratorChar(MessageIds.GDE_MSGT3354));
			importDeviceLogItem.addListener(SWT.Selection, new Listener() {
				@Override
				public void handleEvent(Event e) {
					ZeroAdapter.log.log(java.util.logging.Level.FINEST, "importDeviceLogItem action performed! " + e); //$NON-NLS-1$
					importCsvFiles();
				}
			});
		}
	...
	public void updateFileExportMenu(Menu exportMenue) {
		...
		if (exportMenue.getItem(exportMenue.getItemCount() - 1).getText().equals(Messages.getString(gde.messages.MessageIds.GDE_MSGT0732))) {
			...
			convertKMZ3DRelativeItem.setText(Messages.getString(MessageIds.GDE_MSGT3361));
			...
			convertKMZDAbsoluteItem.setText(Messages.getString(MessageIds.GDE_MSGT3362));
			...
			convertKMZDAbsoluteItem = new MenuItem(exportMenue, SWT.PUSH);
			convertKMZDAbsoluteItem.setText(Messages.getString(MessageIds.GDE_MSGT3363));
			convertKMZDAbsoluteItem.addListener(SWT.Selection, new Listener() {
				@Override
				public void handleEvent(Event e) {
					log.log(Level.FINEST, "convertKMZDAbsoluteItem action performed! " + e); //$NON-NLS-1$
	 ...
	 */
	@Override
	public ZeroAdapterDialog getDialog() {
		return this.dialog;
	}
	...
	public void addDataBufferAsRawDataPoints(RecordSet recordSet, byte[] dataBuffer, int recordDataSize, boolean doUpdateProgressBar) throws DataInconsitsentException {
		...
		ZeroAdapter.log.log(java.util.logging.Level.FINE, timeStamps.size() + " timeStamps = " + timeStamps.toString()); //$NON-NLS-1$

		for (int i = 0; i < recordDataSize; i++) {
			ZeroAdapter.log.log(java.util.logging.Level.FINER, i + " i*dataBufferSize+timeStampBufferSize = " + i * dataBufferSize + timeStampBufferSize); //$NON-NLS-1$
	...
	public String[] prepareDataTableRow(RecordSet recordSet, String[] dataTableRow, int rowIndex) {
		try {
			...
			ZeroAdapter.log.log(java.util.logging.Level.SEVERE, e.getMessage(), e);
		}
		return dataTableRow;
	}
	...
	public double translateValue(Record record, double value) {
		...
		ZeroAdapter.log.log(java.util.logging.Level.FINE, "for " + record.getName() + " in value = " + value + " out value = " + newValue); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		return newValue;
	}
	...
	public void updateVisibilityStatus(RecordSet recordSet, boolean includeReasonableDataCheck) {
		...
		for (int i = 0; i < recordSet.size(); ++i) {
			...
			if (ZeroAdapter.log.isLoggable(java.util.logging.Level.FINE)) ZeroAdapter.log.log(java.util.logging.Level.FINE, record.getName() + " = " + measurementNames[i]); //$NON-NLS-1$

			if (includeReasonableDataCheck) {
				record.setDisplayable(record.hasReasonableData());
				if (ZeroAdapter.log.isLoggable(java.util.logging.Level.FINE))
					ZeroAdapter.log.log(java.util.logging.Level.FINE, i + " " + record.getName() + " hasReasonableData = " + record.hasReasonableData()); //$NON-NLS-1$ //$NON-NLS-2$
			}

			if (record.isActive() && record.isDisplayable()) {
				if (ZeroAdapter.log.isLoggable(java.util.logging.Level.FINE)) ZeroAdapter.log.log(java.util.logging.Level.FINE, "add to displayable counter: " + record.getName()); //$NON-NLS-1$
				++displayableCounter;
			}
		}
		if (ZeroAdapter.log.isLoggable(java.util.logging.Level.FINER)) ZeroAdapter.log.log(java.util.logging.Level.FINER, "displayableCounter = " + displayableCounter); //$NON-NLS-1$
	...
	public void importCsvFiles() {
		final FileDialog fd = FileUtils.getImportDirectoryFileDialog(this, Messages.getString(MessageIds.GDE_MSGT3350));

		Thread reader = new Thread("reader") { //$NON-NLS-1$
			@Override
			public void run() {
				try {
					ZeroAdapter.this.application.setPortConnected(true);
					for (String tmpFileName : fd.getFileNames()) {
						String selectedImportFile = fd.getFilterPath() + GDE.STRING_FILE_SEPARATOR_UNIX + tmpFileName;
						ZeroAdapter.log.log(java.util.logging.Level.FINE, "selectedImportFile = " + selectedImportFile); //$NON-NLS-1$

						if (fd.getFileName().length() > 4) {
							try {
								RecordSet activeRecordSet = ZeroReaderWriter.read(ZeroAdapter.this.getDataBlockSeparator().value().charAt(0), selectedImportFile, ZeroAdapter.this.getRecordSetStemNameReplacement());
								if (activeRecordSet != null) {
									ZeroAdapter.this.updateVisibilityStatus(activeRecordSet, true);
									activeRecordSet.descriptionAppendFilename(selectedImportFile);
								}
							}
							catch (Throwable e) {
								ZeroAdapter.log.log(java.util.logging.Level.WARNING, e.getMessage(), e);
							}
						}
					}
				}
				finally {
					ZeroAdapter.this.application.setPortConnected(false);
				}
			}
		};
		reader.start();
	...
	public void export2KMZ3D(int type) {
		...
		new FileHandler().exportFileKMZ(Messages.getString(MessageIds.GDE_MSGT3360), findRecordByType(activeRecordSet, Record.DataType.GPS_LONGITUDE),
	 ...
	 * refer to LogView Zero format description http://www.logview.info/lvstudio_doku/header.htm
	 * @return true
	 */
	@Override
