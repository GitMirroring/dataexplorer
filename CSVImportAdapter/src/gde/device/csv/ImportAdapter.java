...
/**
 * generic CSV data adapter device implementation class
 */
public class ImportAdapter extends DeviceConfiguration implements IDevice {
	final static Logger			log					= Logger.getLogger(ImportAdapter.class.getName());
	 ...
	 */
	public ImportAdapter(String xmlFileName) throws FileNotFoundException, JAXBException {
		super(xmlFileName);
		// initializing the resource bundle for this device
		Messages.setDeviceResourceBundle("gde.device.csv.messages", Settings.getInstance().getLocale(), this.getClass().getClassLoader()); //$NON-NLS-1$
	 ...
	 * @throws JAXBException
	 */
	public ImportAdapter(String xmlFileName, Unmarshaller tmpUnmarshaller) throws FileNotFoundException, JAXBException {
	 ...
	 * @param deviceConfig
	 */
	public ImportAdapter(DeviceConfiguration deviceConfig) {
		super(deviceConfig);
		// initializing the resource bundle for this device
		Messages.setDeviceResourceBundle("gde.device.csv.messages", Settings.getInstance().getLocale(), this.getClass().getClassLoader()); //$NON-NLS-1$
	...
	private void readProperties() {
		DataAccess.getInstance().checkMappingFileAndCreate(this.getClass(), "CSVMeasurementMappings.xml");
		try (InputStream stream = DataAccess.getInstance().getMappingInputStream("CSVMeasurementMappings.xml")) {
			ImportAdapter.properties.loadFromXML(stream);
		} catch (Exception e) {
			String preopertyFilePath = Settings.MAPPINGS_DIR_NAME + GDE.STRING_FILE_SEPARATOR_UNIX + "CSVMeasurementMappings.xml"; //$NON-NLS-1$
	...
	public void updateVisibilityStatus(RecordSet recordSet, boolean includeReasonableDataCheck) {
			...
			if (this.getMeasurement(channelConfigNumber, i).getProperty(MeasurementPropertyTypes.SCALE_SYNC_REF_ORDINAL.value()) != null) {
				...
				while (iterator.hasNext()) {
					...
					if (propertyType.getName().equals(MeasurementPropertyTypes.SCALE_SYNC_REF_ORDINAL.value())) {
						if (log.isLoggable(Level.FINE)) log.log(Level.FINE, "remove synch from " + measurementNames[i]);
						iterator.remove();
						continue;
					}
				}
		...
		for (int i = 0; i < measurementNames.length; i++) {
			for (int j = i; j < measurementNames.length; j++) {
				...
				if (log.isLoggable(Level.FINER)) log.log(Level.FINER, "check " + measurementNames[j] + " to " + measurementNames[i]);
				if (nameParts.length > 1 &&  measurementNames[i].split(GDE.STRING_BLANK).length > 1 && measurementNames[i].split(GDE.STRING_BLANK)[1].equals(nameParts[1]) && i != j
					...
					if (log.isLoggable(Level.FINE)) log.log(Level.FINE, "do synch " + measurementNames[j] + " to " + measurementNames[i]);
	...
	public void importCsvFiles() {
		...
		Thread reader = new Thread("reader") { //$NON-NLS-1$
			...
			public void run() {
				try {
					ImportAdapter.this.application.setPortConnected(true);
					for (String tmpFileName : fd.getFileNames()) {
						...
						if (fd.getFileName().length() > 4) {
							try {
								RecordSet activeRecordSet = CSVReaderWriter.read(ImportAdapter.this.getDataBlockSeparator().value().charAt(0), selectedImportFile, ImportAdapter.this.getRecordSetStemNameReplacement());
								ImportAdapter.this.updateVisibilityStatus(activeRecordSet, true);
					...
					ImportAdapter.this.application.setPortConnected(false);
				}
			}
		};
		reader.start();
